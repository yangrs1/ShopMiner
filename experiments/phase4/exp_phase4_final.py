"""
Phase 4 FINAL — Fair comparison
v5 Stacking vs my XGBoost_spw on SAME data
"""
...
# Re-load data and build features (same as exp_phase4_hybrid.py)
...
print("="*70); print("FINAL: v5 Stacking vs my XGBoost_spw on SAME data"); print("="*70)
results = {}

# Method 1: v5 Stacking (LGBM + XGB + CatBoost → LR) — same as v5
print("\n[1] v5 Stacking (LGBM + XGB + CatBoost → LR)")
estimators = [
    ...
    ("cat", CatBoostClassifier(iterations=500, learning_rate=0.05, depth=6, verbose=False, random_seed=42, thread_count=-1)),
]
m_stack = StackingClassifier(estimators=estimators, final_estimator=LogisticRegression(max_iter=2000, random_state=42),
                              cv=5, n_jobs=1, passthrough=False)
m_stack.fit(X_tr, y_tr)
yp = m_stack.predict_proba(X_te)[:, 1]
auc = roc_auc_score(y_te, yp)
oot_aucs = []
for we in ["2011-06-30", "2011-07-31", "2011-08-31"]:
    Xo, yo = build_oot_full(we)
    ypo = m_stack.predict_proba(Xo)[:, 1]
    oot_aucs.append(roc_auc_score(yo, ypo))
results["v5_Stacking"] = {"test_auc": float(auc), "oot_mean": float(np.mean(oot_aucs)), "oot_aucs": [float(a) for a in oot_aucs]}
print(f"  Test AUC={auc:.4f}  OOT_mean={np.mean(oot_aucs):.4f}  OOT_each={[f'{a:.3f}' for a in oot_aucs]}")

# Method 2: XGBoost + scale_pos_weight (my best)
print("\n[2] XGBoost + scale_pos_weight (my proposal)")
spw = (y_tr == 0).sum() / (y_tr == 1).sum()
m_xgb = xgb.XGBClassifier(n_estimators=500, learning_rate=0.05, max_depth=6, scale_pos_weight=spw,
                           subsample=0.8, colsample_bytree=0.8, random_state=42, n_jobs=-1, eval_metric="logloss", verbosity=0)
m_xgb.fit(X_tr, y_tr)
yp = m_xgb.predict_proba(X_te)[:, 1]
auc = roc_auc_score(y_te, yp)
oot_aucs = []
for we in ["2011-06-30", "2011-07-31", "2011-08-31"]:
    Xo, yo = build_oot_full(we)
    ypo = m_xgb.predict_proba(Xo)[:, 1]
    oot_aucs.append(roc_auc_score(yo, ypo))
results["XGBoost_spw"] = {"test_auc": float(auc), "oot_mean": float(np.mean(oot_aucs)), "oot_aucs": [float(a) for a in oot_aucs]}
print(f"  Test AUC={auc:.4f}  OOT_mean={np.mean(oot_aucs):.4f}  OOT_each={[f'{a:.3f}' for a in oot_aucs]}")

# Method 3: LightGBM + scale_pos_weight (simpler baseline)
print("\n[3] LightGBM + scale_pos_weight")
m_lgb = lgb.LGBMClassifier(n_estimators=500, learning_rate=0.05, num_leaves=31, max_depth=6,
                            scale_pos_weight=spw, subsample=0.8, colsample_bytree=0.8,
                            random_state=42, n_jobs=-1, verbose=-1)
m_lgb.fit(X_tr, y_tr)
yp = m_lgb.predict_proba(X_te)[:, 1]
auc = roc_auc_score(y_te, yp)
oot_aucs = []
for we in ["2011-06-30", "2011-07-31", "2011-08-31"]:
    Xo, yo = build_oot_full(we)
    ypo = m_lgb.predict_proba(Xo)[:, 1]
    oot_aucs.append(roc_auc_score(yo, ypo))
results["LightGBM_spw"] = {"test_auc": float(auc), "oot_mean": float(np.mean(oot_aucs)), "oot_aucs": [float(a) for a in oot_aucs]}
print(f"  Test AUC={auc:.4f}  OOT_mean={np.mean(oot_aucs):.4f}  OOT_each={[f'{a:.3f}' for a in oot_aucs]}")

# Method 4: Ensemble (XGBoost + LightGBM) — simple average
print("\n[4] Ensemble (XGBoost + LightGBM, soft average)")
m_xgb_ens = xgb.XGBClassifier(n_estimators=500, learning_rate=0.05, max_depth=6, scale_pos_weight=spw,
                               subsample=0.8, colsample_bytree=0.8, random_state=42, n_jobs=-1, eval_metric="logloss", verbosity=0)
m_lgb_ens = lgb.LGBMClassifier(n_estimators=500, learning_rate=0.05, num_leaves=31, max_depth=6,
                                scale_pos_weight=spw, subsample=0.8, colsample_bytree=0.8,
                                random_state=42, n_jobs=-1, verbose=-1)
m_xgb_ens.fit(X_tr, y_tr)
m_lgb_ens.fit(X_tr, y_tr)
yp = 0.5 * m_xgb_ens.predict_proba(X_te)[:, 1] + 0.5 * m_lgb_ens.predict_proba(X_te)[:, 1]
auc = roc_auc_score(y_te, yp)
oot_aucs = []
for we in ["2011-06-30", "2011-07-31", "2011-08-31"]:
    Xo, yo = build_oot_full(we)
    ypo = 0.5 * m_xgb_ens.predict_proba(Xo)[:, 1] + 0.5 * m_lgb_ens.predict_proba(Xo)[:, 1]
    oot_aucs.append(roc_auc_score(yo, ypo))
results["XGB_LGB_Ensemble"] = {"test_auc": float(auc), "oot_mean": float(np.mean(oot_aucs)), "oot_aucs": [float(a) for a in oot_aucs]}
print(f"  Test AUC={auc:.4f}  OOT_mean={np.mean(oot_aucs):.4f}  OOT_each={[f'{a:.3f}' for a in oot_aucs]}")

# Summary
print("\n" + "="*70)
print("FINAL COMPARISON (same data, same split, same OOT windows)")
print("="*70)
print(f"{'Method':<25} {'TestAUC':>8} {'OOT_mean':>9} {'Best?':>10}")
for name, r in results.items():
    mark = "***" if r["oot_mean"] == max(x["oot_mean"] for x in results.values()) else ""
    print(f"  {name:<23} {r['test_auc']:>8.4f} {r['oot_mean']:>9.4f}  {mark}")
print(f"\n  Reference v5 baseline (orig pkl): test=0.918  OOT=0.631")

# Winner
best_name = max(results, key=lambda k: results[k]["oot_mean"])
print(f"\n  WINNER: {best_name}  OOT_AUC={results[best_name]['oot_mean']:.4f}")

# Save the winner
with open(os.path.join(EXP, "phase4_final.json"), "w") as f:
    json.dump({"results": results, "winner": best_name}, f, indent=2)
print(f"\nSaved -> {EXP}/phase4_final.json")
