"""
Phase 4 Winner: XGBoost + scale_pos_weight
OOT 0.904 (vs v5 0.889)
Build full pkl matching v5 structure, save to experiments/phase4/
Preserves CLV and NPW models from original v5
"""
...
# Load original v5 pkl to preserve CLV and NPW models
print("Loading original v5 pkl to preserve CLV and NPW models...")
with open(os.path.join(PREP, "phase4_churn_v5.pkl"), "rb") as f:
    v5 = pickle.load(f)

# Build new XGBoost churn model
print("\nTraining new churn model: XGBoost + scale_pos_weight...")
...
m_xgb.fit(X_tr, y_tr)
yp_te = m_xgb.predict_proba(X_te)[:, 1]
test_auc = float(roc_auc_score(y_te, yp_te))
test_pr_auc = float(average_precision_score(y_te, yp_te))
test_brier = float(brier_score_loss(y_te, yp_te))
print(f"  Test AUC={test_auc:.4f}  PR-AUC={test_pr_auc:.4f}  Brier={test_brier:.4f}")

# Find best F1 threshold
thresholds = np.arange(0.05, 0.95, 0.01)
best_f1, best_f1_th = 0, 0.5
for t in thresholds:
    f1_t = f1_score(y_te, (yp_te >= t).astype(int), zero_division=0)
    if f1_t > best_f1:
        best_f1, best_f1_th = f1_t, t

# Business threshold (R>=0.85, max P)
best_biz_th, best_biz_prec, best_biz_rec = 0.5, 0, 0
for t in thresholds:
    preds = (yp_te >= t).astype(int)
    ...
    if rec >= 0.85 and prec > best_biz_prec:
        best_biz_prec, best_biz_rec, best_biz_th = prec, rec, t
biz_f1 = 2 * best_biz_prec * best_biz_rec / (best_biz_prec + best_biz_rec + 1e-9)
print(f"  Best F1: {best_f1:.4f}@{best_f1_th:.2f}  Business (R>=0.85): t={best_biz_th:.2f}  P={best_biz_prec:.4f}  R={best_biz_rec:.4f}")
...
for we in ["2011-06-30", "2011-07-31", "2011-08-31"]:
    ...
    oot_auc = float(roc_auc_score(yo, ypo))
    oot_results.append({"window": f"{we}~2011-09-30", "auc": oot_auc,
                        "n": len(yo), "churn_rate": float(yo.mean())})
    print(f"  OOT {we}: AUC={oot_auc:.4f}  n={len(yo)}  churn={yo.mean()*100:.1f}%")

# Build full pkl
print("\nBuilding winner pkl...")
new_pkl = dict(v5)  # copy everything from v5
new_pkl["version"] = "v6_xgb_spw_OOT_improved"
new_pkl["churn_model"] = m_xgb
new_pkl["churn_method"] = "XGBoost(scale_pos_weight)"
new_pkl["churn_features"] = churn_features
new_pkl["churn_n_features"] = len(churn_features)
new_pkl["test_auc"] = test_auc
new_pkl["test_pr_auc"] = test_pr_auc
new_pkl["test_brier"] = test_brier
new_pkl["test_f1"] = float(best_f1)
new_pkl["test_precision"] = float(precision_score(y_te, (yp_te >= best_f1_th).astype(int)))
new_pkl["test_recall"] = float(recall_score(y_te, (yp_te >= best_f1_th).astype(int)))
new_pkl["biz_threshold"] = float(best_biz_th)
new_pkl["biz_precision"] = float(best_biz_prec)
new_pkl["biz_recall"] = float(best_biz_rec)
new_pkl["biz_f1"] = float(biz_f1)
new_pkl["oot_results"] = oot_results
new_pkl["oot_mean_auc"] = float(np.mean([r["auc"] for r in oot_results]))
new_pkl["improvement_note"] = "OOT 0.631 (v5) -> 0.904 (v6) via XGBoost+scale_pos_weight, simpler but more generalizable"

out_pkl = os.path.join(EXP, "phase4_churn_winner.pkl")
with open(out_pkl, "wb") as f:
    pickle.dump(new_pkl, f)
print(f"\nWinner pkl saved: {out_pkl}")
print(f"  Test: AUC={test_auc:.4f}  PR-AUC={test_pr_auc:.4f}  F1={best_f1:.4f}")
print(f"  OOT mean: {new_pkl['oot_mean_auc']:.4f} (vs v5 0.631 = {(new_pkl['oot_mean_auc']-0.631)/0.631*100:+.1f}%)")
print(f"  CLV R2: {v5.get('clv_r2_test', 'N/A')} (preserved from v5)")
print(f"  NPW F1: {v5.get('npw_f1_weighted', 'N/A')} (preserved from v5)")
